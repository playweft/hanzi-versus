...
import {
  ...
  canDistract,
  guessMembership,
} from "../src/games/poetry/engine.mjs";
const poems = JSON.parse(
  readFileSync(new URL("../public/data/poetry-curated.json", import.meta.url)),
);
let seed = 42;
const random = () =>
...
test("curated bank contains traceable distinct poems and familiar staples", () => {
  ...
  assert.ok(poems.reduce((n, p) => n + p.lines.length, 0) >= 1500);
  assert.equal(new Set(poems.map((p) => p.id)).size, poems.length);
  for (const title of [
    "静夜思",
    "春晓",
    "登鹳雀楼",
    ...
    assert.ok(poems.some((p) => p.title === title));
  poems.forEach((p, i) => {
    assert.ok(p.lines.length);
    assert.ok(
      p.lines.every((l) =>
        /^(?:[\u3400-\u9fff]{5}|[\u3400-\u9fff]{7})$/.test(l),
      ),
    );
    assert.ok(p.source.file && p.selection.anchor);
...
test("random questions keep all answer tiles, exact board sizes and best overlap sources", () => {
  ...
  for (let i = 0; i < 100; i++) {
    const q = makeQuestion(poems, random);
    sizes.add(q.answer.length);
    sourceCounts.add(q.sources.length);
    assert.equal(q.tiles.length, q.answer.length === 5 ? 9 : 12);
    assert.equal(overlap(q.answer, q.tiles), q.answer.length);
    assert.ok(
      validAnswers(poems, q.tiles, q.answer.length).some(
        (a) => a.line === q.answer,
      ),
    );
    for (const source of q.sources)
      assert.ok(overlap(source.line, q.tiles) < source.line.length);
    assert.equal(
      overlap(q.sources[0].line, q.tiles),
      Math.min(
        q.answer.length - 1,
        overlap(q.answer, q.sources[0].line) + (q.answer.length === 5 ? 4 : 5),
      ),
    );
    const max = Math.max(
      ...poems
        .filter((p) => p.id !== q.poemId && canDistract({ tier: q.tier }, p))
        .flatMap((p) =>
          p.lines
            .filter(
              (l) =>
                l !== q.answer &&
                l.length === q.answer.length &&
                overlap(q.answer, l) < l.length,
            )
            .map((l) => overlap(q.answer, l)),
        ),
    );
    assert.equal(overlap(q.answer, q.sources[0].line), max);
  }
  assert.deepEqual([...sizes].sort(), [5, 7]);
  assert.ok([...sourceCounts].every((n) => n === 1 || n === 2));
});
...
test("joint coverage is optimal and sources cannot complete each other", () => {
  ...
  assert.equal(
    lines.reduce((sum, l) => sum + overlap(l, board), 0),
    8,
  );
...
test("additional copies of common characters count only when needed", () => {
  ...
  extras.forEach((_, i) =>
    assert.equal(
      overlap(line, [...answer, ...extras.filter((_, j) => i !== j)]),
      3,
    ),
  );
});
...
test("consecutive questions prefer different authors", () => {
  ...
  for (let i = 0; i < 20; i++)
    assert.notEqual(
      makeQuestion(poems, random, previous.id).author,
      previous.author,
    );
});
...
test("default mix uses tier weights rather than bank sizes", () => {
  ...
  assert.ok(counts.basic > 3200 && counts.basic < 3800);
  assert.ok(counts.normal > 5200 && counts.normal < 5800);
  assert.ok(counts.advanced > 800 && counts.advanced < 1200);
});

test("poem weight tracks line count below four, then holds flat", () => {
  assert.deepEqual(
    [1, 2, 3, 4, 5, 8, 16].map((n) =>
      poemWeight({ lines: Array(n).fill("字") }),
    ),
    [1, 2, 3, 4, 4, 4, 4],
  );
  ...
  assert.deepEqual(
    weights.map((w, i) => w / pool[i].lines.length),
    [1, 1, 1, 1, 4 / 6],
  );
  ...
  pool.forEach((poem, i) => {
    ...
    const scripted = () => {
      calls++;
      return calls === 1 ? 0 : (lower + upper) / 2;
    };
...
test("all lines are distinct and restored versions match familiar text", () => {
  ...
  assert.ok(lines.includes("千里黄云白日曛"));
  assert.ok(lines.includes("应怜屐齿印苍苔"));
  assert.ok(
    poems.every((p) => ["basic", "normal", "advanced"].includes(p.tier)),
  );
...
test("leftover slots recruit a second source instead of duplicate padding", () => {
  ...
  const candidates = [
    { poem: { id: "one" }, line: "闺中少妇不知愁" },
    { poem: { id: "two" }, line: "此夜曲中闻折柳" },
  ];
  const { sources, extras } = selectDistractors(
    answer,
    candidates,
    5,
    () => 0.1,
  ...
  const score = (tiles) =>
    sources.reduce((n, s) => n + overlap(s.line, tiles), 0);
  extras.forEach((_, i) =>
    assert.ok(
      score([...answer, ...extras.filter((_, j) => j !== i)]) < score(board),
    ),
  );
});
...
test("school and familiar pools admit only explicitly reviewed lines", () => {
  const school = JSON.parse(
    readFileSync(new URL("../scripts/poetry-school.json", import.meta.url)),
  );
  ...
  for (const line of [
    ...
    assert.equal(
      poems.find((p) => p.lines.includes(line))?.tier,
      "basic",
      line,
    );
  for (const line of ["腹有诗书气自华", "人生到处知何似", "应似飞鸿踏雪泥"])
    assert.equal(
      poems.find((p) => p.lines.includes(line))?.tier,
      "normal",
      line,
  ...
  assert.equal(
    poems.find((p) => p.lines.includes("泉眼无声惜细流")).title,
    "小池",
  );
  assert.equal(
    poems.find((p) => p.lines.includes("梅子金黄杏子肥")).author,
    "范成大",
  );
  assert.ok(
    poems
      .find((p) => p.id === "school-high-15")
      .lines.includes("隔篱呼取尽余杯"),
  );
});
...
test("primary decoy stays one character short before adding a secondary decoy", () => {
  ...
  const candidates = [
    { poem: { id: "one" }, line: "曾经沧海难为水" },
    { poem: { id: "two" }, line: "海上明月共潮生" },
  ];
  for (let i = 0; i < 50; i++) {
    const { sources, extras } = selectDistractors(
      answer,
...
test("a primary decoy that needs every slot does not recruit a secondary", () => {
  const candidates = [
    { poem: { id: "one" }, line: "甲己庚辛壬" },
    { poem: { id: "two" }, line: "乙己庚癸子" },
  ];
  const { sources, extras } = selectDistractors(
    "甲乙丙丁戊",
    candidates,
...
test("secondary fill preserves the primary omission drawn before it", () => {
  ...
  for (let seed = 1; seed <= 30; seed++) {
    ...
    const missing = [...first.line].filter(
      (c) => ![...answer, ...primary].includes(c),
    );
...
test("overlap remains strict while tied familiarity uses weighted random chances", () => {
  const tied = ["basic", "normal", "advanced"].map((tier) => ({
    poem: { tier },
    score: 2,
  }));
  const counts = { basic: 0, normal: 0, advanced: 0 };
  for (let i = 0; i < 14000; i++)
    counts[rankDistractors(tied, random)[0].poem.tier]++;
  for (const [tier, weight] of [
    ["basic", 4],
    ["normal", 2],
    ["advanced", 1],
  ]) {
    assert.ok(Math.abs(counts[tier] / 14000 - weight / 7) < 0.02);
  }
  const stronger = { poem: { tier: "advanced" }, score: 3 };
  for (let i = 0; i < 100; i++)
    assert.equal(rankDistractors([...tied, stronger], random)[0], stronger);
});
test("each equally familiar tied line has its own chance, including lines from the same poem", () => {
  const shared = { id: "one", tier: "basic" };
  const candidates = [
    { poem: shared, line: "a", score: 2 },
    { poem: shared, line: "b", score: 2 },
    { poem: { id: "two", tier: "basic" }, line: "c", score: 2 },
  ];
  const counts = { a: 0, b: 0, c: 0 };
  for (let i = 0; i < 6000; i++)
    counts[rankDistractors(candidates, random)[0].line]++;
  Object.values(counts).forEach((n) =>
    assert.ok(Math.abs(n / 6000 - 1 / 3) < 0.03),
  );
});
...
test("decoys never come from a less familiar pool than the target", () => {
  ...
  for (const tier of ["basic", "normal", "advanced"])
    assert.ok(canDistract(p("advanced"), p(tier)));
  const tiers = new Set();
  for (let i = 0; i < 120; i++) {
    const q = makeQuestion(poems, random);
    tiers.add(q.tier);
...
test("advanced guess feedback counts membership, not positions, and caps duplicates", () => {
  assert.deepEqual(guessMembership("床前明月光", "前床明山雨"), [
    true,
    true,
    true,
    false,
  ...
  assert.deepEqual(guessMembership("床前明月光", "床床床月光"), [
    true,
    false,
    false,
    true,
  ...
  assert.deepEqual(guessMembership("人人甲乙丙", "人甲人丁人"), [
    true,
    true,
    true,
    false,
