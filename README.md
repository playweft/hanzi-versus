...
- **成语寻踪**：点击开始后请求 AI 二字线索，保留原有单人及双人规则。
- **拾字成诗**：无需 AI。每题先随机抽一首，再抽其中完整的五言或七言句；从其他诗词中找重字最多的同长度句，随机取前一或两首作为干扰来源。按字符出现次数联合搜索四个或五个干扰字，最大化整组字对干扰句的总覆盖，同时保证每条干扰句至少缺一个字；覆盖相同时优先让缺字位于末尾三字。有剩余位置时，先尝试第二首或替换第二首干扰诗，优先让每个新增字都增加联合覆盖；只有遍历候选仍无法有效填满时，才用不会补全干扰句的字安全补位。重复字为独立字块。
